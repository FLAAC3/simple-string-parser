...
class Parser private constructor(
    ...
    //根据key的长度从大到小排序
    private val keyLengthList: List<Int> = run {
        val set = hashSetOf<Int>()
        dataMap.keys.forEach { set.add(it.length) }
    ...
    fun parse (format: String): String {
        ...
        fun search (index: Int) {
            for (i in index..<format.length) {
                ...
                for (keyLength in keyLengthList) {
                    val endIndex = i + 1 + keyLength
                    if (endIndex > format.length) continue
                    val keyStr = format.substring(i + 1, endIndex)
                    val data = dataMap[keyStr]
                    if (data != null) {
                        if (filter != null && filter!!(keyStr)) continue
                        val search = SearchSubRange(endIndex, format)
                        val value = search.getSubString(data)
                        if (value == null) {
                            expressionList.add((i to endIndex - 1) to data)
                        } else {
